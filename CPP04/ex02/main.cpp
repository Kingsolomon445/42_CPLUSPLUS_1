...
int main()
{
    ...
    dynamic_cast<Dog*>(const_cast<AAnimal*>(animal[3]))->getBrain()->setIdea("You get luckier if best prepared!", 0);
    dynamic_cast<Cat*>(const_cast<AAnimal*>(animal[4]))->getBrain()->setIdea("My family(Cat family) is one of the most sucessful animals on planet!", 0);
    const Dog *dog = new Dog(*(dynamic_cast<Dog*>(const_cast<AAnimal*>(animal[3]))));
    const Cat *cat = new Cat(*(dynamic_cast<Cat*>(const_cast<AAnimal*>(animal[4]))));
    Dog dog2;
    Cat cat2;
    dog2 = *dog;
    cat2 = *cat;
